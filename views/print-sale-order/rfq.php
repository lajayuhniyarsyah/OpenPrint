...
<html>
	...
	<script type="text/javascript">
		jQuery(function(){
			...
			jQuery.each(data,function(index,value){
				...
				if(tinggiContent<600){
					...
					if (tinggiContent ===0){
						contentElement.append("<table width='100%' style='border-collapse:collapse;' border='1px'><th style='text-align:center' width='30px'>No</th><th width='44px' style='text-align:center'>Qty</th><th style='text-align:center' width='44px'>Unit</th><th width='400px' style='text-align:center'>Description"+"</th><th width='130px' style='text-align:center'>Price Unit <?php echo'<br/>'.$model->pricelist->name ?></th><th style='text-align:center'>Price Sub<?php echo'<br/>'.$model->pricelist->name ?></th></table>");
		...
		// $no++;	
					...
					contentElement.append("<table width='100%' style='border-collapse:collapse; margin-top:-1px;'border='1px' >"+"<tr>"+"<td width='30px' valign='top'>"+value.no+"</td>"+"<td valign='top' width='44px'>"+value.product_uom_qty+"</td>"+"<td width='44px' valign='top'>"+value.unit+"</td>"+"<td width='400px' contenteditable='true'>"+"["+value.default_code+"]"+value.name_product+"<br/>"+value.deskription_orderline+"<div class='isi-"+noMaterial+"'></div>"+"</td>"+"<td valign='top' width='130px'>"+value.unit_price+"</td>"+"<td valign='top'>"+value.price_sub+"</td>"+"</tr></table>");
						...
						jQuery.each(value.material_line,function(indexMaterial,valueMaterial){
							...
							if (indexMaterial==0){
								...
								materialElement.append("<ul  style='margin-top:-16px'><li>"+"["+valueMaterial.partNumber+"]"+valueMaterial.product_id+"("+valueMaterial.qty+" "+valueMaterial.uom+")"+"<br/>"+valueMaterial.descriptionMaterial+"</li></ul>")
									...
									jQuery('page#page-'+cursor+' .content').append("<table width='100%' border='1px' style='border-collapse:collapse';><th style='text-align:center' width='30px'>No</th><th width='44px' style='text-align:center'>Qty</th><th style='text-align:center' width='44px'>Unit</th><th width='400px' style='text-align:center'>Description"+"</th><th width='130px' style='text-align:center'>Price Unit<?php echo'<br/>'.$model->pricelist->name ?></th><th style='text-align:center'>Price Sub<?php echo'<br/>'.$model->pricelist->name ?></th></table>");
									if (indexMaterial==0){
										contentElement.append("<table border='1px' width='100%' style='border-collapse:collapse ; margin-top:-1px;' >"+"<tr>"+"<td width='30px'>"+value.no+"</td>"+"<td width='44px'>"+value.product_uom_qty+"</td>"+"<td width='44px'>"+value.unit+"</td>"+"<td width='400px' contenteditable='true'>"+"["+value.default_code+"]"+value.name_product+"Consist of:<ul style='margin-top:0px'>"+"<div class='isi-"+noMaterial+"'></div>"+"</ul>"+"</td>"+"<td width='130px'>"+value.unit_price+"</td>"+"<td>"+value.price_sub+"</td>"+"</tr></table>");
										...
										contentElement.append("<div class=test><table border='1px' width='100%' style='border-collapse:collapse ; margin-top:-1px;'; >"+"<tr>"+"<td width='30px'>"+"</td>"+"<td width='44px'>"+"</td>"+"<td width='44px'>"+"</td>"+"<td width='400px' contenteditable='true'>"+"<div class='isi-"+noMaterial+"'></div>"+"</td>"+"<td width='130px'>"+"</td>"+"<td>"+"</td>"+"</tr></table></div>");
										var contentElement = jQuery('page#page-'+cursor+' .content');
										var tinggiContent = contentElement.height();
										var materialElement = jQuery('page#page-'+cursor+' .content'+' .isi-'+noMaterial);
			...
			if (tinggiContent<=261){
				contentfooter.append("<table width='45%'><tr><td width='40%'><b>Term Of Payment</b></td><td width='3%'><b>:</b></td><td><?php if($model->paymentTerm!==null){echo $model->paymentTerm->name;}?></td></tr><tr><td valign='top'><b>Term Condition</b></td><td valign='top'><b>:</b></td><td><?php echo '<ul>';foreach ($model->termConditions as $key_term => $value_term) {	echo '<li>'.$value_term->name.'</li>';}echo '</ul>'?></td></tr><tr><td><b>Note</b></td><td><b>:</b></td><td><?= $model->note?></td></tr></table><br/><table width='30%'><tr> <td valign='top' style='text-align:center' height='80'>Thank you and best regards</td></tr><tr><td valign='top' style='text-align:center'>"+"<?=$model->user->name ?>"+"</td></tr></table>");
