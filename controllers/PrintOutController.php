...
class PrintOutController extends Controller
{
	...
	public function actionPrintInvoice($id,$uid=null,$printer="refa",$printForFaktur=false)
	{
		...
		# account invoice
		$modelAccInvoice = $oe->read([$id],['id','kwitansi','date_invoice','amount_untaxed','amount_untaxed_main','amount_tax','amount_total','amount_total_main','approver','partner_id','currency_id','invoice_line','pajak','comment','name','payment_for','payment_term','faktur_pajak_no'],"account.invoice");
	...
	**/
	// action print
	/*public function actionPrintFaktur($id,$uid=null,$printer=null,$block=False,$printForFaktur=False){
		if($block){
		...
		# account invoice
		$modelAccInvoice = $oe->read([$id],['id','kwitansi','date_invoice','amount_untaxed','amount_untaxed_main','amount_tax','amount_total','amount_total_main','approver','partner_id','currency_id','invoice_line','pajak','comment','name','payment_for','payment_term','faktur_pajak_no','kmk'],"account.invoice");
		// var_dump($modelAccInvoice);
		...
		# res partner
		$modelPartner = $oe->read([$idPartner],['id','name','street','street2','city','state_id','zip','npwp'],"res.partner");
		$partner = $modelPartner[0];
		// var_dump($partner);
		...
		if($model['currency_id'][1]=='IDR' and $model['currency_id'][0]==13)
		{
			// if Rupiah
			return $this->render('account-invoice/faktur_rupiah',[
				'model'=>$model,
				'data'=>$data,
				'uid'=>$uid,
				'printer'=>$printer,
				'modelPartner'=>$partner,
				'invoice'=>$invoice,
				'modelAccInvoiceLines'=>$modelAccInvoiceLines
			]);
		} 
		else {
			return $this->render('account-invoice/faktur_valas',[
				'model'=>$model,
				'data'=>$data,
				'uid'=>$uid,
				'printer'=>$printer,
