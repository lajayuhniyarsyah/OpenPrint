<html>
	...
	<script type="text/javascript">
		jQuery(function(){
			...
			var contentHeader = "<table width=100% class=tableHeader><td valign=top witdh=50%><table width=100%><tr><td width=119>Company<td width=5>:<td width=275><?=$model->partner->name?><tr><td>Attn<td>:<td><?=$AttentionName?><tr valign=top><td>Delivery Address<td>:<td height=54><?=$street_shipping.' '?><?=$street_shipping_2.' '?><?=$city_shipping.' '?><?=$state_shipping.' '?><?=$country_shipping.' '?><tr valign=top><td>Invoice Address<td>:<td height=54><?=$street_invoice.' '?><?=$street_invoice_2.' '?><?=$city_invoice.' '?><?=$state_invoice.' '?><?=$country_invoice.' '?><tr><td>Phone<td>:<td><?= $AttentionPhone?><tr><td>Fax<td>:<td><?=$fax ?><tr><td>Email<td>:<td><?= $email?></table><td width=50% valign=top><table width=100%><tr><td width=140>Quotation#<td width=5>:<td width=253><?=$model->quotation_no?><tr><td>Customer Reference#<td>:<td><?=$model->client_order_ref?><tr><td>Date<td>:<td><?=Yii::$app->formatter->asDate($model->date_order);?><tr><td>Due Date</td><td>:<td><?=$model->due_date ? Yii::$app->formatter->asDate($model->due_date):null?><tr><td>Delivery Due Date<td>:<td><?=$model->delivery_date ? Yii::$app->formatter->asDate($model->delivery_date):null;?><tr><td>Sales Contact<td>:<td><?=$sales?><tr><td>Sales Group<td>:<td><?=strtoupper($model->group->name)?></table></table>"
